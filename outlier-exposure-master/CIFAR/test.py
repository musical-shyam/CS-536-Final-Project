...
if __name__ == '__main__':
    ...
    # /////////////// Textures ///////////////
    dtd_path = get_dataset_path("dtd")
    ood_data = dset.ImageFolder(root=os.path.join(dtd_path, 'images'),
    ...
    #/////////////// SVHN ///////////////
    # svhn_path = get_dataset_path("svhn")
    ...
    # lsun_path = get_dataset_path("LSUN")
    # ood_data = lsun_loader.LSUN(root = lsun_path, classes='test',
    #                             transform=trn.Compose([trn.Resize(32), trn.CenterCrop(32),
    #                                                 trn.ToTensor(), trn.Normalize(mean, std)]))
    # ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
    #                                         num_workers=args.prefetch, pin_memory=True)
    ...
    if 'cifar10_' in args.method_name:
        ood_data = dset.CIFAR100(root=get_dataset_path("cifar-100"), train=False, transform=test_transform, download=True)
    else:
        ood_data = dset.CIFAR10(root= get_dataset_path("cifar-10"), train=False, transform=test_transform)
    ...
    if 'cifar10_' in args.method_name:
        ...
        ood_data = dset.CIFAR10(root= get_dataset_path('cifar-10'), train=False, transform=test_transform, download=True)
    ...
    if 'cifar10_' in args.method_name:
        ood_data = dset.CIFAR100(root= get_dataset_path('cifar-100'), train=False, transform=trn.ToTensor(), download=True)
    else:
        ood_data = dset.CIFAR10(root= get_dataset_path('cifar-10'), train=False, transform=trn.ToTensor(), download=True)
